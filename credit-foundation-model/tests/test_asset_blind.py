...
BANNED_MODULE_MARKERS = ("mortgage", "dutch", "reference_implementations")
...
def test_no_asset_files_inside_the_package():
    ...
    offenders = [str(p.relative_to(REPO)) for p in (REPO / "src" / "credit_fm").rglob("*")
                 ...
                 and any(m in p.name.lower() for m in ("mortgage", "dutch"))]
    ...
    for py in (REPO / "src" / "credit_fm").rglob("*.py"):
        for i, line in enumerate(py.read_text().splitlines(), 1):
            ...
            if ("import" in stripped
                    and any(m in stripped for m in ("mortgage", "dutch"))):
...
def test_reference_implementation_registers_via_the_contract():
    ...
    try:
        importlib.import_module("reference_implementations.mortgage_performance")
        from credit_fm.data.adapter import REGISTRY
        assert "mortgage_performance" in REGISTRY
    finally:
