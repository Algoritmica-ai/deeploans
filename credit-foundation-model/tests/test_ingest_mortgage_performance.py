...
_spec = importlib.util.spec_from_file_location(
    "mortgage_adapter",
    Path(__file__).resolve().parent.parent / "reference_implementations" / "mortgage_performance" / "adapter.py")
...
def _adapter(stage: dict):
    """Build a MortgagePerformanceAdapter with a stub DatasetConfig (source resolution needs no contract)."""
    ...
    return ing.MortgagePerformanceAdapter(cfg, stage=stage)
...
def test_adapter_load_panel_end_to_end_local(tmp_path):
    """MortgagePerformanceAdapter on local parquet sources: derives, samples, concatenates — no GCS."""
    raw = pd.DataFrame({
