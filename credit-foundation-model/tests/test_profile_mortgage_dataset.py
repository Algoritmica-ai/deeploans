...
G = _load("reference_implementations/mortgage_performance/mortgage_glossary.py", "mortgage_glossary")
...
def _schema_cols() -> list[tuple[int, str]]:
    out = []
    for line in (ROOT / "configs/mortgage_performance/raw_schema.yaml").read_text().splitlines():
...
def _run_profiler(panel: Path, out: Path) -> dict:
    subprocess.run(
        [sys.executable, str(ROOT / "scripts/profile_mortgage_dataset.py"),
