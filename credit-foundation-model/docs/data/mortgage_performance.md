...
## Leakage discipline (same rules as Dutch — see `configs/mortgage_performance/baseline.yaml`)
...
python scripts/ingest_mortgage_performance.py --files data/raw/<file1>.parquet data/raw/<file2>.parquet

# B. real run: a multi-year reporting span from GCS (covers obs_date + horizon)
python scripts/ingest_mortgage_performance.py \
    --gcs-root gs://sriram-credit-fm-data/raw_by_reporting \
...
python scripts/prepare_data.py --input data/raw/mortgage_performance/panel.parquet \
...
python scripts/classify_schema.py --input data/raw/mortgage_performance/panel.parquet \
    --out configs/mortgage_performance/tokenizer.yaml
# honest Gate-G1 baseline, reading the splits straight from the bucket
python scripts/train_baseline.py --config configs/mortgage_performance/baseline.yaml \
    --data-dir gs://sriram-credit-fm-data/processed/mortgage_performance/run_2016_2017 \
    --report reports/mortgage_baseline_report.md
```
...
## Derived columns (in `ingest_mortgage_performance.py`)
